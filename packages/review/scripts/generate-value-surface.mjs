#!/usr/bin/env node
/**
 * Write the committed goldens of the review package's public RUNTIME (value) surface — the
 * name/kind sets that `spec/reviewBarrelValueSurface.spec.ts` and
 * `spec/reviewDeepSubpathValueSurface.spec.ts` compare against. The probes themselves live in
 * `value-surface.mjs` and `deep-subpath-surface.mjs`, which bind core's shared engine to this
 * package and are the same modules those specs import — so the files this writes are by
 * construction the things the specs derive rather than a second opinion about them.
 *
 * **Both goldens, one command.** The root barrel and the gated source modules move together often
 * enough that a second command is a command to forget. Every failure message in both specs names
 * this one.
 *
 * Run AFTER a build, because the probes import the emitted package in `dist/`:
 *
 *   pnpm --filter @gaunt-sloth/review run build
 *   pnpm --filter @gaunt-sloth/review run value-surface:generate
 *
