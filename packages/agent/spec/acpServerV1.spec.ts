...
interface AgentScript {
   ...
   *
   * Scripted rather than detected: the detector is a core concern with its own cells, and what this
   * surface has to prove is only that the fact travels through the real runner and the real agent
   * app to a client that would otherwise be told nothing but `end_turn`.
   */
...
describe('the ACP v1 agent — session lifecycle', () => {
  ...
  /**
   * [[EXT-158]] — **the ending v1's stop-reason union cannot express either.**
   *
   * `end_turn` is what an editor is told both when the agent finished and when it stopped halfway
   * through its own checklist. v1 carries the termination TAXONOMY structurally on `_meta`, which
   * is a closed vocabulary a client can switch on; this is a different kind of fact, so it goes to
   * the conversation channel and nowhere else. Asserted here because deleting the emission from the
   * app leaves every core cell green.
   */
