#!/usr/bin/env node
/**
 * Write the committed golden of the JUnit reporter package's public RUNTIME (value) surface — the
 * name/kind set that `spec/junitBarrelValueSurface.spec.ts` compares against. The probe itself
 * lives in `value-surface.mjs`, which binds core's shared engine to this package and is the same
 * module that spec imports — so the file this writes is by construction the thing the spec derives
 ...
 * `spec/junitPublishedSurface.spec.ts` asserts it still holds.
 *
 * Run AFTER a build, because the probe imports the emitted package in `dist/`:
 *
 *   pnpm --filter @gaunt-sloth/eval-reporter-junit run build
 *   pnpm --filter @gaunt-sloth/eval-reporter-junit run value-surface:generate
 *
